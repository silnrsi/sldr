...
root_sldr = os.path.join(os.path.dirname(os.path.dirname(__file__)), "sldr")
    ...
    #need to find a way to deal with inheritence, or at least empty files that only contain an ID block. Don't need those. 
...
for x in missingdata: 
    ...
    if cldr_stat == 'only' and x.get("cldr") == False:
        continue
    elif cldr_stat == 'excl' and x.get("cldr") == True:
        continue
    if ltag != None:
...
fil_out = "Output Parameters: "
if args.get('langtag') != None:
    search += "Single Langtag [" + str(args.get('langtag')) + "], "
elif args.get('langtag') == None:
    search += "Multiple SLDR files filtered by: "
    if args.get('range') != None:
        search += "Alphabetical Sections of SLDR [" + str(args.get('range')) + "], "
    if args.get('territory') != None:
        search += "Region Code [" + str(args.get('territory')) + "], "
    if args.get('script') != None:
        search += "Script Code [" + str(args.get('script')) + "], "
    if args.get('CLDR') == 'incl':
        search += "Include CLDR Files in Search"
    elif args.get('CLDR') == 'excl':
...
if args.get('coverage') == ('basic' or 'both'):
    if len(filter) == 0:
        ...
        fil_out += "Missing Basic Requirements, with all categories"
    else:
        fil_out += "Missing Basic Requirements that fall under the following categories: " + str(filter)
            ...
            if alldata[file][cat] != None:        
                ...
                if all not in filter or cat != "basic_msng": 
                    ...
                    for msng in alldata[file][cat]:
                        if results == 'list':    
                            print(filt_indt + "\t\t" + alldata[file][cat][msng])
                            #msng_written += alldata[file][cat][msng] + ", "
                        elif results == 'ref':
                            print(filt_indt + "\t\t" + msng)
                            #msng_written += msng + ", "
                        elif results == 'full':
...
#FOR CODE IMPLIMENT LATER, IF TIME IS 24 HR TIME THEN NO AM PM STUFF NEEDED. ALSO UPPERCASE H IS 24 HR AND THE AMOUNT OF LETTERS HAS TO DO WITH ZERO/NON ZERO BUFFER OR SOMETHING? TEST MIGHT NEED TO BE A BIT MORE FLEXIBLE
    
#order of "filtering out" if statements needs to be: 
...
####################################################
####################################################
#OLD CODE I'M NOT DELETING YET UNTIL I KNOW I DONT NEED TO REFERENCE IT AGAIN
####################################################
